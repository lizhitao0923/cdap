...
public class DatasetInstanceService {
  ...
  DatasetMeta get(final DatasetId instance, List<? extends EntityId> owners) throws Exception {
    ...
    if (!DatasetsUtil.isSystemDatasetInUserNamespace(instance)) {
      LOG.info("---- Authorizing GET for dataset {}", instance.getDataset());
      ...
      LOG.info("---- Authorized GET for dataset {}", instance.getDataset());
    }
    ...
    try {
      LOG.info("---- Retrieving instance metadata from cache for dataset {}", instance.getDataset());
      datasetMeta = metaCache.get(instance);
      LOG.info("---- Retrieved instance metadata from cache for dataset {}", instance.getDataset());
  ...
  private DatasetMeta getFromMds(DatasetId instance) throws Exception {
    ...
    LOG.info("---- Retrieving instance metadata from MDS for dataset {}", instance.getDataset());
    DatasetSpecification spec = instanceManager.get(instance);
    if (spec == null) {
      throw new NotFoundException(instance);
    }
    LOG.info("---- Retrieved instance metadata from MDS for dataset {}", instance.getDataset());
    ...
    if (!NamespaceId.SYSTEM.equals(instance.getNamespaceId())) {
      LOG.info("---- Retrieving owner principal for dataset {}", instance.getDataset());
      ownerPrincipal = ownerAdmin.getOwnerPrincipal(instance);
      LOG.info("---- Retrieved owner principal for dataset {}", instance.getDataset());
    }
  ...
  void create(String namespaceId, String name, DatasetInstanceConfiguration props) throws Exception {
    ...
    if (!DatasetsUtil.isSystemDatasetInUserNamespace(datasetId)) {
      LOG.info("---- Authorizing impersonation for dataset {}", name);
      ...
      LOG.info("---- Authorized impersonation for dataset {}", name);
    }

    LOG.info("---- Ensuring existence of namespace {} for dataset {}", namespace, name);
    ensureNamespaceExists(namespace);
    LOG.info("---- Ensured existence of namespace {} for dataset {}", namespace, name);

    LOG.info("---- Retrieving instance metadata from MDS for dataset {}", name);
    DatasetSpecification existing = instanceManager.get(datasetId);
    if (existing != null) {
      throw new DatasetAlreadyExistsException(datasetId);
    }
    LOG.info("---- Retrieved instance metadata from MDS for dataset {}", name);
    ...
    try {
      ...
      LOG.info("---- Calling op executor service to configure dataset {}", name);
      DatasetSpecification spec = opExecutorClient.create(datasetId, typeMeta, datasetProperties);
      LOG.info("---- Received spec from op executor service for dataset {}: ", name, spec);

      LOG.info("---- Adding instance metadata for dataset {}", name);
      instanceManager.add(namespace, spec);
      LOG.info("---- Added instance metadata for dataset {}", name);
      metaCache.invalidate(datasetId);

      LOG.info("---- Publishing audit for creation of dataset {}", name);
      publishAudit(datasetId, AuditType.CREATE);
      LOG.info("---- Published audit for creation of dataset {}", name);
      if (publishCUD && !isLocalDataset(datasetProperties.getProperties()) && !isLocalDataset(spec.getProperties())) {
        LOG.info("---- Publishing metadata for creation of dataset {}", name);
        metadataPublisher.publish(datasetId, DatasetInstanceOperation.create(requestingUser,
                                                                             props.getTypeName(), datasetProperties));
        LOG.info("---- Published metadata for creation of dataset {}", name);
      }
  ...
  private DatasetTypeMeta getTypeInfo(NamespaceId namespaceId, String typeName, boolean byPassCheck) throws Exception {
    ...
    try {
      LOG.info("---- Retrieving metadata from mds for dataset type {} with authorization: {}", typeName, byPassCheck);
      ...
      LOG.info("---- Retrieved metadata from mds for dataset type {}", typeName);
      return meta;
      ...
      try {
        ...
        LOG.info("---- Retrieving metadata from mds for system dataset type {}", typeName);
        DatasetTypeId systemDatasetTypeId = ConversionHelpers.toDatasetTypeId(NamespaceId.SYSTEM, typeName);
        LOG.info("---- Retrieved metadata from mds for system dataset type {}", typeName);
  ...
  private void enableExplore(DatasetId datasetInstance, DatasetSpecification spec,
    ...
    // Note: today explore enable is not transactional with dataset create - CDAP-1393
    try {
      LOG.info("---- Enabling explore for dataset {}", datasetInstance.getDataset());
      exploreFacade.enableExploreDataset(datasetInstance, spec, false);
      LOG.info("---- Enabled explore for dataset {}", datasetInstance.getDataset());
    } catch (Exception e) {
